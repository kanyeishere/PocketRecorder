...
- 录制游戏画面，并在可用时捕获系统音频。
- NVIDIA 环境下优先尝试 NativeRecorder GPU 纹理路径输出 MP4；不可用或失败时自动回退 FFmpeg。
...
当前版本通过 PresentHook 直接获取游戏 D3D11 backbuffer。NVIDIA + `NativeRecorder.dll` 可用时，Present hook 只把 backbuffer 快速复制到插件自有的 shared texture ring，然后 native DLL 打开 shared handle、在 GPU 上转 NV12，并优先通过 NvEncoderD3D11 + libavformat 写 HEVC MP4，不再经过 `ffmpeg.exe` stdin rawvideo。若 NvEncoder/libavformat 初始化失败，native 内部会先尝试 Media Foundation fallback；若 native 路径不可用或首帧提交失败，则自动回到原有 FFmpeg 路径。
...
与 OBS Game Capture 相比，OBS 仍然拥有更成熟的全 GPU shared texture 编码管线；PocketRecorder 的 native 路径采用同类的资源隔离思路，但只服务 FFXIV 录制。FFmpeg fallback 仍保留 NV12 readback 和管道写入，用于兼容非 NVIDIA、native DLL 缺失或驱动/编码器不支持的机器。
