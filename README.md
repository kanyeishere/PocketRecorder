...
- NVIDIA/AMD/Intel 环境下优先尝试 NativeRecorder GPU 纹理路径输出 MP4；不可用或失败时自动回退 FFmpeg。
...
NativeRecorder 现在只打包单个 `NativeRecorder.abi13.dll`。NVIDIA 路径使用 Video Codec SDK 12.x 构建，以降低 NVENC 驱动要求；AMD AMF 和 Intel oneVPL QSV 后端在同一个 DLL 内按游戏实际 D3D11 设备厂商自动选择。

本地测试 SDK 12 可以直接指定 SDK 根目录：

```powershell
dotnet build .\PocketRecorder.csproj -c Debug -p:NativeRecorderNvenc12SdkDir="A:\Downloads\Video_Codec_SDK_12.0.16"
```

GitHub Actions 发布需要配置 `VIDEO_CODEC_SDK_12_URL` secret 或 repository variable，指向可下载的 NVIDIA Video Codec SDK 12.x zip。旧的 `VIDEO_CODEC_SDK_URL` 仍可作为兼容别名，但内容也必须是 SDK 12.x。
