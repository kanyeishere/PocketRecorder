...
```text
Present hook GPU copy -> shared D3D11 texture -> native D3D11 video processor NV12 conversion -> Media Foundation H.264 sink writer -> MP4
```
...
- Stable C ABI v3 is defined in `include/pocket_recorder_native.h`.
- `pr_probe` reports D3D11 texture support on NVIDIA machines with `nvEncodeAPI64.dll` and Media Foundation available.
- `pr_submit_d3d11_shared_texture` accepts the source D3D11 device plus a D3D11 shared texture handle, creates the native device on the same adapter, performs GPU NV12 conversion, and writes H.264 MP4 through Media Foundation.
...
1. Prove the current Media Foundation H.264 path across common NVIDIA GPUs and resolutions.
2. Replace the internal encoder with `NvEncoderD3D11` if Media Foundation shows driver-specific limitations.
3. Add HEVC/AV1 once the H.264 path is stable.
